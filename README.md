...
第一阶段 GUI 原型，默认加载 mock 数据。已支持通过 Tauri 手动扫描自定义字体目录的基础文件信息，字体内部元数据解析、数据库等后端能力仍在后续阶段。
...
- 字体搜索与排序
- 自定义字体目录扫描（基础文件级别，并通过本地字体文件参与预览渲染）
- 字体预览：横排/竖排、字号/字重/行距/字间距调节、预设文本
- 字符检测（Mock 逻辑）
- 字体信息展示（含 Unicode 覆盖进度条）
- 原始文件名与文件路径查看、复制
- 扫描统计：内部名称读取、文件名兜底与潜在预览风险提示
- 字体库自动保存与启动恢复（Tauri app data JSON）
- 标签管理与备注编辑
- 浅色/深色主题切换
...
- 默认数据来自 mock 数据；手动扫描目录后会以真实字体文件列表替换当前列表。
- 扫描后的字体库会保存到 Tauri app data 目录，下次启动会优先恢复保存的字体库。
- 当前扫描只读取文件路径、名称、格式和大小，尚未解析字体内部元数据。
...
- 系统字体扫描：通过 Tauri invoke 调用 Rust 命令扫描系统字体目录
